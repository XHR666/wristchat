# OPPO Watch X2 AI 助手 **WristChat** — 设计文档 v0.5(待审阅)

> v0.5 变更:思考模式手表端默认「开 + effort 低」(设置可调);进设置页静默检查更新(15 分钟冷却);对话费用计入余额页累计;**上下文自动压缩**(超出阈值直接用默认模型压缩旧消息,不弹提示);上下文窗口=1M(官方)。**本轮重点审阅 §11 剩余问题。**
> 状态:**待你审阅**
...
| **模型/思考** | 模型选择(需先填 Key);思考模式开关(默认开)+ effort(**默认低**);上下文窗口显示(默认 1M)、压缩阈值(默认 85%)、max_tokens(默认 4096) |
| **快捷输入** | 条目增删改 |
...
**Q1 · 自动生成会话标题?**:每次新会话首条消息后,用默认模型把会话自动起个标题(如「关于…的讨论」,计入消耗,约几百 token)
➡️ 推荐:v1 默认**关**(省 token);要的话我加上开关,默认开也行

**Q2 · 压缩触发阈值默认值**:上下文超窗口 **85%** 触发压缩?
➡️ 推荐:85%(1M 窗口下手表对话基本触发不到,但机制完整;设置可调 50–95%)

**Q3 · 压缩摘要的展示**:聊天里插入一条分割线「📌 已压缩前 N 条消息」,摘要可展开查看?
➡️ 推荐:是(透明可见,不藏内容)

**Q4 · 更新检查冷却时间**:进设置页自动检查的冷却默认 **15 分钟**?
➡️ 推荐:15 分钟(可改 5–60)

**已定死(本轮):**
- 仓库名 `XHR666/wristchat`(你 A1 确认)✅
- 思考默认「开 + effort 低」,设置可调 ✅
- 进设置自动静默检查,冷却防重复 ✅
- 对话费用计入余额页累计 ✅
- 上下文超出 → **自动压缩**,不弹提示;压缩模型=默认模型 ✅
- 上下文窗口 1M(官方核对)✅
